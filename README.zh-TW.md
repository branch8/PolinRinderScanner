...
| `polinrider-quickstart-mac.command` | **非技術人員（macOS）** — 雙擊即可執行，自動下載最新掃描器 |
| `polinrider-quickstart-windows.bat` | **非技術人員（Windows）** — 雙擊即可執行，自動下載最新掃描器 |
...
給不熟悉終端機操作的團隊成員使用。不需要 git clone，不需要打指令，下載後直接雙擊即可。

### macOS — `polinrider-quickstart-mac.command`

1. 下載 `polinrider-quickstart-mac.command` 到任何地方（例如桌面）
2. **第一次使用：** 右鍵 → **「開啟」** → 再點一次「開啟」（繞過 Gatekeeper）
3. 之後直接雙擊即可
4. Terminal 自動開啟，下載最新掃描器，執行全機掃描，結果停在畫面上供閱讀

### Windows — `polinrider-quickstart-windows.bat`

1. 下載 `polinrider-quickstart-windows.bat` 到任何地方（例如桌面）
2. 雙擊執行，不需要任何額外設定
3. PowerShell 自動開啟，下載最新掃描器，執行全機掃描，結果停在畫面上供閱讀

> 兩隻啟動器每次執行都會從 GitHub 下載最新版掃描器，無需手動更新。
